...
public class Tarefas extends AppCompatActivity implements TaskAdaptativo.OnTaskClickListener {
    ...
    private void showPopupMenu(View view) {
        ...
        popupMenu.getMenuInflater().inflate(R.menu.tarefas_menu, popupMenu.getMenu());
        popupMenu.setOnMenuItemClickListener(item -> {
            int id = item.getItemId();
            if (id == R.id.action_mark_all_complete) {
                new AlertDialog.Builder(this)
                    .setTitle("Marcar Todas Como Concluídas")
                    .setMessage("Tem certeza que deseja marcar todas as tarefas como concluídas?")
                    .setPositiveButton("Sim", (dialog, which) -> {
                        String currentDate = dateFormat.format(calendar.getTime());
                        List<Task> tasksToUpdate = baseDados.getTasksByDate(currentDate);
                        for (Task task : tasksToUpdate) {
                            if (!task.isCompleted() && task.getId() != -1) {
                                baseDados.updateTaskCompletionStatus(task.getId(), true);
                            }
                        }
                        loadTasksAndScroll();
                        Toast.makeText(Tarefas.this, "Todas as tarefas foram marcadas como concluídas.", Toast.LENGTH_SHORT).show();
                    })
                ...
                new AlertDialog.Builder(this)
                    ...
                    .setMessage("Tem certeza que deseja deletar todas as tarefas do dia?")
                    .setPositiveButton("Deletar", (dialog, which) -> {
                        String currentDate = dateFormat.format(calendar.getTime());
                        List<Task> tasksToDelete = baseDados.getTasksByDate(currentDate);
                        ...
                        loadTasksAndScroll();
                        Toast.makeText(Tarefas.this, "Todas as tarefas foram deletadas", Toast.LENGTH_SHORT).show();
                    })
